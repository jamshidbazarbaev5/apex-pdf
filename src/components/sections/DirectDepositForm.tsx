      ...
      <DocumentSheet>
        ...
        <div className="mb-10 border-2 border-black">
          ...
          <div className="bg-white">
            ...
            <div className="flex h-12">
                <div className="w-[230px] pl-3 pr-2 border-r border-black flex items-center shrink-0 bg-white">
                  <span className="text-[13px] font-bold text-black uppercase font-serif tracking-wide leading-none pt-1">ACCOUNT TYPE</span>
                </div>
                 <div className="flex-1 bg-transparent flex items-center px-5 gap-10">
                     ...
                     <label className="flex items-center cursor-pointer gap-3 select-none">
                        <div className="relative w-[18px] h-[18px] border border-black bg-white flex items-center justify-center">
                          <input
                            type="checkbox"
                            name="vehicleAirRide"
                            checked={Boolean(formData.vehicleAirRide || false)}
                            onChange={handleChange}
                            className="peer appearance-none w-full h-full cursor-pointer absolute inset-0 z-10"
                          />
                          {formData.vehicleAirRide && (
                             <svg className="w-5 h-5 text-black pointer-events-none relative -top-1 left-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                               <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                             </svg>
                          )}
                        </div>
                        <span className="text-[13px] font-bold uppercase font-serif tracking-wide pt-1">CHECKING</span>
                     </label>
                     ...
                     <label className="flex items-center cursor-pointer gap-3 select-none">
                        <div className="relative w-[18px] h-[18px] border border-black bg-white flex items-center justify-center">
                          <input
                            type="checkbox"
                            name="vehicleDockHigh"
                            checked={Boolean(formData.vehicleDockHigh || false)}
                            onChange={handleChange}
                            className="peer appearance-none w-full h-full cursor-pointer absolute inset-0 z-10"
                          />
                          {formData.vehicleDockHigh && (
                             <svg className="w-5 h-5 text-black pointer-events-none relative -top-1 left-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="3">
                               <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                             </svg>
                          )}
                        </div>
                        <span className="text-[13px] font-bold uppercase font-serif tracking-wide pt-1">SAVINGS</span>
                     </label>
                 </div>
            </div>
          </div>
        ...
        <div className="mb-16 mt-12 px-2">
          <p className="text-justify text-[17px] leading-[1.6] font-serif text-black">
             I authorize AXPER LLC (<span className="underline">1673 Reed Dr, Krum, TX, 76249, US</span>) to initiate 
             credit entries to the account indicated above for the purpose of expense and/or 
             payroll. I also authorize AXPER LLC to initiate, if necessary, debit entries and 
             adjustments for any credit entries made in error. This authorization is to remain 
             in force until the AXPER LLC has received written authorization from me of 
        ...
        <div className="mt-12 space-y-10 max-w-[550px]">
          <div>
            <label className="text-[18px] font-serif text-black mb-3 block">Vendor Signature</label>
            <SignButton
